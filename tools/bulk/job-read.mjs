...
// that has none.
// config: { glyphs: [set | a+b union, …], ladder, tol, quant, palette, shadow, matchcols, union }
...
async function read(item, config, sets, bytes, doc) {
  ...
  for (let pno = 1; pno <= pages; pno++) {
    ...
    if (config.ladder) {
      ...
      const ran = new Map();
      const NOBODY = Symbol('nobody reads this page');
      let got;
      try {
      ...
      } catch (e) { if (e !== NOBODY) throw e; const first = ran.get('0|0|0'); got = { res: first.r, pass: first.p }; }
